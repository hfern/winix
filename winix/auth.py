...
def login(username: str, password: str, **kwargs):
    ...
    return WinixAuthResponse(
        user_id=jwt.get_unverified_claims(resp['AuthenticationResult']['AccessToken'])['sub'],
...
def _boto_client(region):
    ...
    return boto3.client('cognito-idp', config=Config(signature_version=UNSIGNED), region_name=region or COGNITO_REGION)
